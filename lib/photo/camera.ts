 ...
 */
export async function openCamera({ facing = "user" }: { facing?: CameraFacing } = {}): Promise<MediaStream> {
  if (!canUseLiveCamera()) {
...
async function drawToJpegFile(
  ...
  return new File([blob], name ?? `photo-${Date.now()}.jpg`, { type: "image/jpeg" })
}
...
export async function captureVideoFrame(video: HTMLVideoElement): Promise<File> {
  ...
  return drawToJpegFile(video, sourceWidth, sourceHeight, { name: `eleve-${Date.now()}.jpg` })
}
