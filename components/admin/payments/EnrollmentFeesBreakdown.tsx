...
> = {
  paid: {
    label: FEE_STATUS_LABEL.paid,
    icon: CheckCircle2,
    dot: "bg-emerald-500",
    text: "text-emerald-600 dark:text-emerald-400",
  },
  partial: {
    label: FEE_STATUS_LABEL.partial,
    icon: CircleDot,
    dot: "bg-amber-500",
    text: "text-amber-600 dark:text-amber-400",
  },
  pending: {
    label: FEE_STATUS_LABEL.pending,
    icon: Circle,
    dot: "bg-muted-foreground/40",
    text: "text-muted-foreground",
  },
  waived: {
    label: FEE_STATUS_LABEL.waived,
    icon: MinusCircle,
    dot: "bg-muted-foreground/40",
    text: "text-muted-foreground",
  },
  in_kind: {
    label: FEE_STATUS_LABEL.in_kind,
    icon: Package,
    dot: "bg-sky-500",
...
export function EnrollmentFeesBreakdown({
  ...
  /** Le depot etait une erreur : la ligne redevient due en argent. */
  onCancelDeposit?: (fee: EnrollmentFeeItem) => void;
          ...
          {fees.map((fee) => {
            ...
            return (
              <div key={fee.id} className="py-3 first:pt-1 last:pb-1">
                ...
                {/* Le retour en arriere se trouve sur la ligne meme, la ou
                    l'erreur se voit. Sans lui, la fiche pouvait poser un depot
                    sans jamais pouvoir le defaire : une correction a la main
                    dans la base a deja ete necessaire pour cette raison. */}
                {onCancelDeposit && fee.status === "in_kind" && (
                  <div className="mt-2 flex justify-end">
                    <Button
                      type="button"
                      size="sm"
