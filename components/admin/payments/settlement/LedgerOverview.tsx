...
export function LedgerOverview({
  ...
  const cartes = [...donnees.categories].sort((a, b) => {
    if (a.taux === null && b.taux === null) return a.name.localeCompare(b.name, "fr")
...
function CarteCategorie({
  ...
  return (
    <button
      ...
      aria-label={
        taux !== null
          ? `${categorie.name} : ${arrondi(taux)} % recouvré. Ouvrir le détail.`
          : `${categorie.name} : ${fmt(categorie.total_entre)} ${legende}. Ouvrir le détail.`
      }
      className={cn(
    ...
    >
          ...
          <div className="mt-1 flex flex-wrap items-center gap-1.5 text-[11px] text-muted-foreground">
            ...
            {categorie.accepts_in_kind && (
              <span className="inline-flex items-center gap-1 rounded-full border border-border px-1.5 py-0.5">
                ...
                {categorie.depots_en_nature.toLocaleString("fr-FR")} dépôt
                {categorie.depots_en_nature > 1 ? "s" : ""}
              </span>
            )}
          </div>
        </div>
      ...
      {categorie.eleves_sans_ligne !== null && categorie.eleves_sans_ligne > 0 && (
