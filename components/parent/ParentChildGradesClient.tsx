...
import { useParentChildGrades } from "@/lib/hooks/useParentPortal"
...
interface ParentChildGradesClientProps {
  childId: number
}

export function ParentChildGradesClient({ childId }: ParentChildGradesClientProps) {
  const [trimester, setTrimester] = useState<string | undefined>(undefined)
  const { data, isLoading, isError, refetch } = useParentChildGrades(childId, trimester)
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="font-serif text-xl tracking-tight">
            {data ? `Notes — ${data.child_name}` : "Notes"}
          </h1>
          {data && (
...
function SubjectCard({ subject }: { subject: ParentChildSubjectGrades }) {
  return (
