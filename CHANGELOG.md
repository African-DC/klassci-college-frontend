...
- Mode dictée : le micro se débloque tout seul dès qu'on l'autorise dans le navigateur, sans recharger la page. Quand le micro est bien autorisé mais que la reconnaissance vocale est bloquée par le navigateur (fréquent sur Microsoft Edge), le message invite désormais à utiliser Google Chrome ou à saisir au clavier, au lieu d'afficher « micro refusé » à tort. Sur une adresse non sécurisée (http), un message explique que la dictée vocale exige le site sécurisé https. Un bouton « Réessayer le micro » reste proposé *(enseignant, admin)*.
